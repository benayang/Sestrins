...
condition = unlist(lapply(unlist(sampleTable$sample), function(x){unlist(strsplit(x,split="-"))[4]}))
day = unlist(lapply(unlist(sampleTable$sample), function(x){paste(unlist(strsplit(x,split="-"))[2:3], collapse="-")}))
replicate = unlist(lapply(unlist(sampleTable$sample), function(x){unlist(strsplit(x,split="-"))[5]}))
sampleTable = cbind(sampleTable, 
                    condition=factor(condition, levels=c("WT","KO")), 
                    day=factor(day, levels=c("d-0","d-7","d-21")), 
                    replicate=factor(replicate))
sampleTable = mutate(sampleTable, group = factor(paste(day,condition,sep="-")))
sampleTable$group = relevel(sampleTable$group, ref="d-0-WT")
sampleTable = mutate(sampleTable, path = file.path(".","mm38",paste("MuSC",day,condition,replicate,sep="-")))
sampleTable=sampleTable[order(sampleTable$day),]
sampleTable$batch = factor(c(rep("old",12), rep("new",3)))
sampleTable = sampleTable[-c(11,12,13),] # remove MuSC-d-21-WT-R1
sampleTable
```
...
tiff(filename="./nosva_v4/Figures/dds_spearman.tiff", units="in", width=7, height=5, res=320)
pheatmap(dds_cor, cluster_rows = F, cluster_cols = F, display_numbers = T, main="Spearman Correlation")
dev.off()
```
...
ggsave(dds_pca, filename="./nosva_v4/Figures/rlog_pca_newreps.tiff", 
...
# write.csv(resLFC_d0_Down_Sig0.01, file="./test_d0_sig0.01_down_l2fc1.csv")
# write.csv(resLFC_d0_Up_Sig0.01, file="./test_d0_sig0.01_up_l2fc1.csv")
```
...
# write.csv(resLFC_d7_Down_Sig0.01, file="./test_d7_sig0.01_down_l2fc1.csv")
# write.csv(resLFC_d7_Up_Sig0.01, file="./test_d7_sig0.01_up_l2fc1.csv")
```
...
# write.csv(resLFC_d21_Down_Sig0.01, file="./test_d21_sig0.01_down_l2fc1.csv")
# write.csv(resLFC_d21_Up_Sig0.01, file="./test_d21_sig0.01_up_l2fc1.csv")
```

```{r}
WebGestaltR(enrichMethod = "ORA",
            organism = "mmusculus",
            enrichDatabase = c("geneontology_Biological_Process_noRedundant", "pathway_KEGG"),
            interestGene = readClipboard(),
...
pheatmap(mat, annotation_col = anno_col, cluster_cols = F, scale = "row")
...
tiff(filename="./nosva_v4/Figures/d0_volcano.tiff", units="in", width=7, height=5, res=180)
