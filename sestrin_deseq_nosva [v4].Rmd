...
anno <- as.data.frame(colData(rld)[, c("condition","day")])
...
ggsave(hmp, filename="./nosva_v4/Figures/Top50Var_rld_KOvsWT_pheatmap.tiff", units="in", width=7.25, height=6.75, dpi=320)
```
...
d0_d7_d21_venn = ggVennDiagram(d0_d7_d21_int, lty=1, lwd=0.75, color="black") + theme(text=element_text(size=16, family="sans"))
...
total_geneList_stack = stack(total_geneList)

total_geneList_ens = genes(edb, columns="symbol", filter=SymbolFilter(unlist(total_geneList)), return.type="data.frame")$gene_id
...
levels(gene_df$Cluster) = c("Qui", "NP", "Infl", "Neur", "Adh", "Myo", "Chr", "Wnt", "ROS")
...
```{r}
total_bar = ggbarplot(total_df, x="Symbol", y="log2FoldChange", 
          fill="log10padj", color="day", 
          position=position_dodge(), 
          rotate=TRUE) +
  gradient_fill("RdBu") +
  labs_pubr()
total_bar
ggexport(total_bar, filename="./nosva_v4/Figures/total_bar.tiff")
```

```{r}
d0_bar = ggbarplot(d0_geneList, x="Symbol", y="log2FoldChange",
                    fill = "log10padj", color = "white",
                    sort.val = "asc", sort.by.groups = TRUE,
                    merge = TRUE) + 
  gradient_fill("RdBu") +
...
d7_bar = ggbarplot(d7_geneList, x="Symbol", y="log2FoldChange",
                    fill = "log10padj", color = "white",
                    sort.val = "asc", sort.by.groups = TRUE,
                    merge = TRUE) + 
  gradient_fill("RdBu") +
...
d21_bar = ggbarplot(d7_geneList, x="Symbol", y="log2FoldChange",
                    fill = "log10padj", color = "white",
                    sort.val = "asc", sort.by.groups = TRUE,
                    merge = TRUE) + 
  gradient_fill("RdBu") +
...
all_bar = ggarrange(d0_bar, d7_bar, d21_bar)
all_bar = ggpar(all_bar, font.xtickslab = 5, font.ytickslab = 5, font.x = 5, font.y=5)
all_bar
ggsave(all_bar, filename="./nosva_v4/Figures/all_bar.tiff", width=8, height=9, units="in", dpi=90)
```
...
```
```{r}
ggscatter(total_df,  x="day",y="log2FoldChange", facet.by="Cluster", color="Cluster",
