...
# d0down_wsc_terms
d0down_GOBP_wsc = c("GO:0042303", "GO:0060326", "GO:0001503", "GO:0060537", "GO:0002237", "GO:0009611", "GO:0048285")
d0down_KEGG = c("mmu03010", "mmu04115")
d0down_terms = c(d0down_GOBP_wsc, d0down_KEGG)
d0down_terms
```

```{r}
# all up and down regulated genes in enriched GO:BP and KEGG terms for day 0
time_enrich_df = time_enrich[["d0down"]] %>% dplyr::filter(geneSet %in% d0down_terms)
...
rownames(time_gene_matrix) = c("Molting cycle", "Cell chemotaxis", "Ossification", "Ribosome pathway",
                               "Muscle tissue development", "p53 signaling pathway",
                               "Response to molecule of bacterial origin", "Response to wounding", "Organelle fission") # Geneset vs genes
...
time_gene_matrix$l2fc = down_tab$log2FoldChange[match(as.character(time_gene_matrix$Var1), down_tab$Symbol, nomatch=F)] * time_gene_matrix$value
time_gene_matrix$log10padj = -log10(down_tab$padj[match(as.character(time_gene_matrix$Var1), down_tab$Symbol, nomatch=F)]) * time_gene_matrix$value
...
d0_term_plot
ggsave(d0_term_plot, filename=file.path(projdir,"Figures","d0_wsc_term_plot.tiff"), units="in", width=10, height=4, dpi=320)
```
...
ex <- data.frame(subset(d0_rev_data, subset=lfc=="Up"&dispensability<0.15))
